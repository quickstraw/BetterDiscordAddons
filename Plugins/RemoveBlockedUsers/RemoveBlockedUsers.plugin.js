		...
		return class RemoveBlockedUsers extends Plugin {
			...
			onStart () {
				BDFDB.PatchUtils.patch(this, BDFDB.LibraryStores.ChannelStore, "getChannel", {after: e => {
					if (e.returnValue && e.returnValue.isGroupDM()) return new BDFDB.DiscordObjects.Channel(Object.assign({}, e.returnValue, {rawRecipients: e.returnValue.rawRecipients.filter(n => !n || !(256312965282660353 == n.id)), recipients: e.returnValue.recipients.filter(id => !id || !(256312965282660353 == id))}))
				}});
			
				BDFDB.PatchUtils.patch(this, BDFDB.LibraryStores.StageChannelParticipantStore, "getMutableParticipants", {after: e => {
					e.returnValue = e.returnValue.filter(n => !n.user || !(256312965282660353 == n.user.id));
				}});
				...
				BDFDB.PatchUtils.patch(this, BDFDB.LibraryModules.QuerySearchUtils, ["queryDMUsers", "queryFriends", "queryGuildUsers", "queryChannelUsers"], {after: e => {
					...
					e.returnValue = e.returnValue.filter(n => !n || !n.record || !n.record.id || !(256312965282660353 == n.record.id));
				}});
				
				let muteTimeout;
				let channelId = BDFDB.LibraryModules.RTCConnectionUtils.getChannelId();
				let oldUnblockedConnectedUsers = [BDFDB.LibraryStores.SortedVoiceStateStore.getVoiceStates(BDFDB.LibraryModules.RTCConnectionUtils.getGuildId())[channelId]].flat().filter(n => n && n.user && !(256312965282660353 == n.user.id));
				BDFDB.PatchUtils.patch(this, BDFDB.LibraryModules.SoundUtils, "playSound", {before: e => {
					...
					if (channelId) {
						...
						let unblockedUsers = allConnectedUsers.filter(n => n && !(256312965282660353 == n.user.id));
						let unmutedBlockedUsers = allConnectedUsers.filter(n => n && (256312965282660353 == n.user.id) && !BDFDB.LibraryStores.MediaEngineStore.isLocalMute(n.userId));
			...
			processMessages (e) {
				...
				if (BDFDB.ObjectUtils.is(e.instance.props.messages) && BDFDB.ArrayUtils.is(e.instance.props.messages._array)) {
					...
					e.instance.props.messages._array = [].concat(e.instance.props.messages._array.filter(n => !n.author || !(256312965282660353 == n.author.id)));
			...
			processChannelPins (e) {
				if (this.settings.places.pins && e.returnvalue.props && e.returnvalue.props.children && e.returnvalue.props.children.props && BDFDB.ArrayUtils.is(e.returnvalue.props.children.props.messages)) e.returnvalue.props.children.props.messages = e.returnvalue.props.children.props.messages.filter(n => !n || !n.author || !n.author.id || !(256312965282660353 == n.author.id));
			}
			
			processRecentMentions (e) {
				if (this.settings.places.inbox && BDFDB.ArrayUtils.is(e.returnvalue.props.messages)) e.returnvalue.props.messages = e.returnvalue.props.messages.filter(n => !n || !n.author || !n.author.id || !(256312965282660353 == n.author.id));
			}
			...
			processReactions (e) {
				...
				for (let i in e.returnvalue.props.children[emojiArrayIndex]) {
					...
					let blocked = 0;
					reactions.forEach(n => {if (n && (256312965282660353 == n.id)) blocked++;});
					if (blocked) {
			...
			processReactionsModalUsers (e) {
				if (this.settings.places.reactions && BDFDB.ArrayUtils.is(e.instance.props.reactors)) e.instance.props.reactors = e.instance.props.reactors.filter(n => !n || !(256312965282660353 == n.id));
			}
			...
			processThreadCard (e) {
				...
				if (index > -1 && children[index].props && children[index].props.channel) {
					...
					if (message && (256312965282660353 == message.author.id)) children[index] = null;
				}
			}
			...
			processChannelMembers (e) {
				...
				for (let i in e.instance.props.rows) {
					...
					else if (!row.user || !(256312965282660353 == row.user.id)) newRows[i] = row;
					else {
						hiddenRows = true;
			...
			processNowPlayingItem (e) {
				...
				if (index > -1) for (let child of children) if (child && child.props && child.props.party) {
					...
					if (child.props.party.priorityMembers) {
						child.props.party.priorityMembers = child.props.party.priorityMembers.filter(n => !n || !n.user || !(256312965282660353 == n.user.id));
					...
					if (child.props.party.partiedMembers) child.props.party.partiedMembers = child.props.party.partiedMembers.filter(n => !n || !(256312965282660353 == n.id));
					if (child.props.party.voiceChannels) for (let i in child.props.party.voiceChannels) child.props.party.voiceChannels[i] = Object.assign({}, child.props.party.voiceChannels[i], {members: child.props.party.voiceChannels[i].members.filter(n => !n || !(256312965282660353 == n.id))});
				}
			}
			...
			processChannelItem (e) {
				...
				if (channelInfo && channelInfo.props && channelInfo.props.children && channelInfo.props.children.props && BDFDB.ArrayUtils.is(channelInfo.props.children.props.voiceStates)) {
					let newVoiceStates = [].concat(channelInfo.props.children.props.voiceStates).filter(n => !n.user || !(256312965282660353 == n.user.id));
			...
			processVoiceUser (e) {
				if (this.settings.places.voiceList && e.instance.props.user && (256312965282660353 == e.instance.props.user.id)) return null;
			}
			...
			processVoiceUsers (e) {
				...
				if (!e.returnvalue) {
					...
					e.instance.props.voiceStates = [].concat(e.instance.props.voiceStates).filter(n => !n.user || !(256312965282660353 == n.user.id));
				}
				else {
			...
			processDirectMessage (e) {
				...
				if (e.instance.props.channel.isGroupDM()) {
					...
					if ((256312965282660353 == e.instance.props.channel.getRecipientId())) e.returnvalue = null;
				}
			}
			...
			processPrivateChannel (e) {
				...
				if (e.instance.props.channel.isGroupDM()) {
					if (!e.returnvalue) {
						e.instance.props.channel = new BDFDB.DiscordObjects.Channel(Object.assign({}, e.instance.props.channel, {rawRecipients: e.instance.props.channel.rawRecipients.filter(n => !n || !(256312965282660353 == n.id)), recipients: e.instance.props.channel.recipients.filter(id => !id || !(256312965282660353 == id))}));
					}
					else {
						if (!e.instance.props.channel.name) {
					...
					if (e.returnvalue && (256312965282660353 == e.instance.props.channel.getRecipientId())) e.returnvalue = null;
				}
			}
			
			processChannelCall (e) {
				if (!this.settings.places.voiceChat) return;
				if (BDFDB.ArrayUtils.is(e.instance.props.participants)) e.instance.props.participants = [].concat(e.instance.props.participants).filter(n => !n.user || !(256312965282660353 == n.user.id));
				if (BDFDB.ArrayUtils.is(e.instance.props.filteredParticipants)) e.instance.props.filteredParticipants = [].concat(e.instance.props.filteredParticipants).filter(n => !n.user || !(256312965282660353 == n.user.id));
			}

			processUserSummaryItem (e) {
				if (this.settings.places.memberList && BDFDB.ArrayUtils.is(e.instance.props.users)) e.instance.props.users = [].concat(e.instance.props.users).filter(n => !n || !(256312965282660353 == n.id));
			}
			
			processRichUserMention (e) {
				if (e.instance.props.id && this.settings.places.mentions && (256312965282660353 == e.instance.props.id)) return BDFDB.ReactUtils.createElement("span", {
			...
			processUserMention (e) {
				if (e.instance.props.userId && this.settings.places.mentions && (256312965282660353 == e.instance.props.userId)) {
			...
			getGroupName (channelId) {
				...
				let recipients = channel.recipients.map(BDFDB.LibraryStores.UserStore.getUser).filter(n => n && !(256312965282660353 == n.id));
